...
const Forecast = ({ data }) => {
  ...
  return (
    <>
    <label className="block text-3xl font-bold text-gray-900 mb-6">Daily</label>
<Accordion allowZeroExpanded>
  {data.list.slice(0, 7).map((item, idx) => (
    <AccordionItem key={idx} className="border border-gray-300 rounded-xl mb-4">
      <AccordionItemHeading>
        <AccordionItemButton className="flex justify-between items-center p-6 bg-gray-50 hover:bg-gray-100 rounded-xl">
          <div className="flex items-center space-x-6">
            ...
            />
            <div>
              <label className="block text-lg font-medium text-gray-900">{forecastDays[idx]}</label>
      ...
      <AccordionItemPanel className="p-6 bg-white rounded-xl">
